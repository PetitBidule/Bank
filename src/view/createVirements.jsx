...
const CreateVirements = () => {
    ...
    return (
        <div className="container mx-auto p-4">
                ...
                <div>
                    ...
                    <select name="destination_account" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" required>
                        ...
                        {beneficiaries.map(beneficiary => (
                            <option key={beneficiary.account_beneficiary} value={parseInt(beneficiary.account_beneficiary)}>{beneficiary.bank_account_beneficiary_iban} ({beneficiary.bank_account_beneficiary_name})</option>
                        ))}
                    </select>
                </div>
            ...
            {virementId && (
                <div className="mt-4">
                    <p className="text-green-500">Transaction created successfully! Virement ID: {virementId}</p>
                    <p className="text-gray-500">Time remaining to cancel: {timer} seconds</p>
                    <button onClick={cancelTransaction} className="bg-red-500 text-white px-4 py-2 rounded-md shadow-sm hover:bg-red-600 mt-2">Cancel Virement</button>
                </div>
            )}
        </div>
    );
